...
class TestFillModel(FillModel):
    ...
    def get_fill(self, order: Order, timestamp: datetime, universe: AssetUniverse) -> FillResult:
...
def test_market_order_full_fill(broker, portfolio, start_time):
    ...
    assert portfolio.open_orders[0].status == OrderStatus.FILLED
    assert portfolio.open_orders[0].filled_quantity == 10
...
def test_market_order_insufficient_margin(broker, portfolio, start_time):
    ...
    assert portfolio.open_orders[0].status == OrderStatus.REJECTED
...
def test_limit_order_execution(broker, portfolio, start_time):
    ...
    limit_price = 105.0  # Above AAPL's close price
...
def test_limit_order_expiry(broker, portfolio, start_time):
    ...
    assert portfolio.open_orders[0].status == OrderStatus.EXPIRED
...
def test_custom_fill_model(sample_universe_data, portfolio, start_time):
    ...
    assert portfolio.open_orders[0].filled_price == expected_fill_price
